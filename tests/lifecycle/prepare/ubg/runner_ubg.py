#!/usr/bin/env python3
"""Black-box harness for Ubuntu GNOME bootstrap_ubg.sh Prepare tests."""
...
BOOTSTRAP_SOURCE = REPO_ROOT / "scripts" / "nix" / "ubg" / "bootstrap_ubg.sh"
BASH_EXECUTABLE = shutil.which("bash")
if BASH_EXECUTABLE is None:
    raise RuntimeError("Ubuntu GNOME Prepare lifecycle tests require Bash on PATH")
...
def host_is_supported_ubuntu() -> bool:
    """Return whether the real host satisfies bootstrap_ubg.sh's supported OS check."""
    ...
    return values.get("ID") == "ubuntu" and values.get("VERSION_ID") in {
        "22.04",
        "24.04",
        "26.04",
    }
...
class UbgPrepareHarness:
    """Build and execute isolated Ubuntu GNOME Prepare scenarios."""
    ...
    def _merge(base: dict[str, Any], overrides: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in overrides.items():
            if isinstance(value, dict) and isinstance(merged.get(key), dict):
                merged[key] = UbgPrepareHarness._merge(merged[key], value)
            else:
    ...
    def _placeholder(name: str) -> str:
        return (
            "#!/usr/bin/env bash\n"
            f"# Ubuntu GNOME Prepare test placeholder for {name}\n"
            "exit 0\n"
        )

    def _build_repo_source(self, root: Path) -> Path:
        repo_source = root / "repo-source"
        ubg_dir = repo_source / "scripts" / "nix" / "ubg"
        ubg_dir.mkdir(parents=True)
        (repo_source / ".git").mkdir()
        (repo_source / ".git" / "HEAD").write_text(
            "ref: refs/heads/main\n", encoding="utf-8"
        )
        (repo_source / "managed").mkdir()
        (repo_source / "managed" / "version.txt").write_text(
            "ubg-prepare-characterization\n", encoding="utf-8"
        )
        shutil.copy2(BOOTSTRAP_SOURCE, ubg_dir / "bootstrap_ubg.sh")
        for name in ("setup_ubg.sh", "config_ubg.sh", "verify_ubg.sh", "update_ubg.sh"):
            path = ubg_dir / name
            path.write_text(self._placeholder(name), encoding="utf-8")
        for path in ubg_dir.glob("*.sh"):
            path.chmod(0o644)
        return repo_source
    ...
    def _prepare_fixture(
        ...
        # Create the metadata only inside the isolated runtime fixture.
        ...
        course_ubg = course_root / "scripts" / "nix" / "ubg"
        course_ubg.mkdir(parents=True, exist_ok=True)
        bootstrap_path = course_ubg / "bootstrap_ubg.sh"
        shutil.copy2(BOOTSTRAP_SOURCE, bootstrap_path)
        bootstrap_path.chmod(0o755)

        repo_source = self._build_repo_source(temp_root)
        tmp_dir = temp_root / "tmp"
        tmp_dir.mkdir()
        state = self._merge(self._default_mock_state(), scenario.get("mock_overrides", {}))
        state["repo_source"] = str(repo_source)
        ...
        mock_dir.mkdir()
        self._write_mock_wrapper(mock_dir / "sudo", "sudo")
        if state.get("initial_git", True):
            self._write_mock_wrapper(mock_dir / "git", "git")

        native_dir = temp_root / "native-bin"
        native_dir.mkdir()
        for command in (
            "bash", "chmod", "cp", "date", "find", "grep", "mkdir",
            "mktemp", "rm", "tee", "touch",
        ):
            source = shutil.which(command)
            if source is None:
                raise RuntimeError(f"Ubuntu GNOME Prepare tests require native utility: {command}")
            (native_dir / command).symlink_to(Path(source).resolve())

        return home, tmp_dir, mock_dir, native_dir, state_path, trace_path
    ...
    def _environment(
        ...
        for key, value in scenario.get("environment", {}).items():
    ...
    def _wait_for_log(log_file: Path | None) -> None:
        ...
        for _ in range(80):
            try:
                ...
                if "[SUCCESS] IT 140 automation files were retrieved." in text or "[ERROR]" in text:
                    return
            except OSError:
                pass
            time.sleep(0.05)
    ...
    def _execute(
        ...
        before_logs = set(log_dir.glob("bootstrap_ubg_*.log")) if log_dir.exists() else set()
        env = self._environment(
            home, tmp_dir, mock_dir, native_dir, state_path, trace_path, scenario
        )
        bootstrap_path = home / "it140" / "scripts" / "nix" / "ubg" / "bootstrap_ubg.sh"
        completed = subprocess.run(
            [BASH_EXECUTABLE, str(bootstrap_path)],
            cwd=home,
            env=env,
            text=True,
            capture_output=True,
        ...
        after_logs = set(log_dir.glob("bootstrap_ubg_*.log")) if log_dir.exists() else set()
    ...
    def run_scenario(self, scenario: dict[str, Any]) -> PrepareRun:
        temp_root = Path(tempfile.mkdtemp(prefix="it140-prepare-ubg-"))
        try:
            prepared = self._prepare_fixture(temp_root, scenario)
            return self._execute(scenario, temp_root, *prepared)
        except Exception:
    ...
    def _semantic_state(run: PrepareRun) -> dict[str, Any]:
        ...
        return {
            ...
            "protected": _snapshot(UbgPrepareHarness._protected_paths(home)),
        }

    def run_twice(self, scenario: dict[str, Any]) -> PrepareSequence:
        temp_root = Path(tempfile.mkdtemp(prefix="it140-prepare-ubg-twice-"))
        try:
            prepared = self._prepare_fixture(temp_root, scenario)
