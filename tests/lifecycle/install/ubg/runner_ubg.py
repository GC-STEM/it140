#!/usr/bin/env python3
"""Black-box harness for Ubuntu GNOME setup_ubg.sh Install lifecycle tests."""
...
INSTALL_SOURCE = REPO_ROOT / "scripts" / "nix" / "ubg" / "setup_ubg.sh"
...
if BASH_EXECUTABLE is None:
    raise RuntimeError("Ubuntu GNOME Install lifecycle tests require Bash on PATH")
...
def parse_install_log(path: Path) -> InstallTranscript:
    ...
    for raw_line in text.splitlines():
        line = raw_line.strip("\ufeff\r\n")
        if line == "IT 140 UBUNTU GNOME SETUP SUMMARY":
            in_summary = True
            continue
        ...
        if key in {
            ...
            "Development status", "Manifest release", "Managed changes",
            "Warnings", "Failures", "Elapsed seconds", "Next step",
            "Log file", "Exit code",
        }:
...
class UbgInstallHarness:
    """Build and execute isolated Ubuntu GNOME Install scenarios."""
    ...
    def _merge(base: dict[str, Any], overrides: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in overrides.items():
            if isinstance(value, dict) and isinstance(merged.get(key), dict):
                merged[key] = UbgInstallHarness._merge(merged[key], value)
            else:
    ...
    def _prepare_fixture(
        ...
        ubg_dir = course_root / "scripts" / "nix" / "ubg"
        manifest_dir.mkdir(parents=True, exist_ok=True)
        ubg_dir.mkdir(parents=True, exist_ok=True)
        shutil.copy2(INSTALL_SOURCE, ubg_dir / INSTALL_SOURCE.name)
        ...
        for command in ("curl", "dpkg", "dpkg-query", "gpg", "sudo"):
            self._write_mock_wrapper(mock_dir / command, command)
        for command in (
            "bash", "chmod", "date", "dirname", "flock", "grep", "id",
            "mkdir", "mktemp", "python3", "rm", "tee", "uname",
        ):
            resolved = shutil.which(command)
            if resolved is None:
                raise RuntimeError(f"Required harness command is unavailable: {command}")
    ...
    def _execute(
        ...
        before_logs = set(log_dir.glob("setup_ubg_*.log")) if log_dir.exists() else set()
        env = self._environment(home, system_root, mock_dir, state_path, trace_path)
        install_path = home / "it140" / "scripts" / "nix" / "ubg" / "setup_ubg.sh"
        ...
        after_logs = set(log_dir.glob("setup_ubg_*.log")) if log_dir.exists() else set()
        ...
        return InstallRun(
            ...
            stdout=completed.stdout, stderr=completed.stderr, home=home,
            system_root=system_root, log_dir=log_dir, log_file=log_file,
            transcript=transcript, protected_differences=_differences(before, after),
            trace_file=trace_path, state_file=state_path,
        )

    def run_scenario(self, scenario: dict[str, Any]) -> InstallRun:
        temp_root = Path(tempfile.mkdtemp(prefix="it140-install-ubg-"))
        try:
            home, system_root, mock_dir, state_path, trace_path = self._prepare_fixture(temp_root, scenario)
            return self._execute(scenario, temp_root, home, system_root, mock_dir, state_path, trace_path)
        except Exception:
    ...
    def _semantic_state(run: InstallRun) -> dict[str, Any]:
        ...
        for relative in (
            ...
            system_files[relative] = path.read_text(encoding="utf-8", errors="replace") if path.is_file() else None
        return {"mock_state": state, "system_files": system_files}

    def run_twice(self, scenario: dict[str, Any]) -> InstallSequence:
        temp_root = Path(tempfile.mkdtemp(prefix="it140-install-ubg-twice-"))
        try:
            home, system_root, mock_dir, state_path, trace_path = self._prepare_fixture(temp_root, scenario)
            first = self._execute(scenario, temp_root, home, system_root, mock_dir, state_path, trace_path)
            first_state = self._semantic_state(first)
            second = self._execute(scenario, temp_root, home, system_root, mock_dir, state_path, trace_path)
