...
PY
    ...
    if numlock_is_on; then
        ...
        record_result WARNING verify.numlock_session \
            "Num Lock is not ON in the current Xfce session" \
            "This may be normal for some keyboards. If a numeric keypad behaves unexpectedly, check Num Lock."
    fi
}
