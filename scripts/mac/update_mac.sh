...
readonly IT140_SCRIPT_VERSION="0.5.3"
...
extract_script_version() {
    sed -n 's/^readonly IT140_SCRIPT_VERSION="\([^"]*\)"$/\1/p' "$1" | head -n 1
}

semver_compare() {
    "$IT140_PYTHON_BIN" - "$1" "$2" <<'PYSEMVER'
import sys

def parse(value):
    parts=value.split('.')
    if len(parts)!=3 or any(not p.isdigit() or (len(p)>1 and p[0]=='0') for p in parts): raise SystemExit(2)
...
configuration_complete() {
    ...
    [ -n "$(git config --global --get user.name 2>/dev/null || true)" ] || return 1
    local email
    email="$(git config --global --get user.email 2>/dev/null || true)"
    [[ "$email" == *@users.noreply.github.com ]] || return 1
    grep -Fq "$IT140_MANAGED_ENV_START" "$HOME/.zprofile" 2>/dev/null || return 1
    grep -Fq "$IT140_MANAGED_ENV_START" "$HOME/.zshrc" 2>/dev/null || return 1
    [ -x "$IT140_VENV_PYTHON" ] || return 1
    [ -r "$HOME/Library/Application Support/Code/User/settings.json" ] || return 1
    [ -L "$IT140_COURSE_DESKTOP_LINK" ] || return 1
    [ -x "$IT140_VSCODE_DESKTOP_APP/Contents/MacOS/open-it140-in-code" ] || return 1
    return 0
}

begin_transaction() {
    IT140_TRANSACTION_ROOT="$IT140_SCRIPT_ROOT/.it140-transaction.$$"
    rm -rf "$IT140_TRANSACTION_ROOT"
    mkdir -p "$IT140_TRANSACTION_ROOT/backups" "$IT140_TRANSACTION_ROOT/staged"
    : > "$IT140_TRANSACTION_ROOT/records.tsv"
    chmod 0700 "$IT140_TRANSACTION_ROOT"
}

stage_asset() {
    local source="$1" destination="$2" mode="$3" relative="$4"
    local staged="$IT140_TRANSACTION_ROOT/staged/$relative"
    local backup="$IT140_TRANSACTION_ROOT/backups/$relative"
    mkdir -p "$(dirname "$staged")" "$(dirname "$backup")"
    cp "$source" "$staged"
    chmod "$mode" "$staged"
    if [ -f "$destination" ]; then
        cp -p "$destination" "$backup"
        printf '%s\t%s\tpresent\n' "$destination" "$backup" >> "$IT140_TRANSACTION_ROOT/records.tsv"
    else
        printf '%s\t%s\tabsent\n' "$destination" "$backup" >> "$IT140_TRANSACTION_ROOT/records.tsv"
    fi
}
...
commit_transaction() {
    IT140_TRANSACTION_ACTIVE=1
    local destination backup state relative staged
    while IFS=$'\t' read -r destination backup state; do
        relative="${backup#"$IT140_TRANSACTION_ROOT/backups/"}"
        staged="$IT140_TRANSACTION_ROOT/staged/$relative"
        mkdir -p "$(dirname "$destination")"
        mv "$staged" "$destination"
    done < "$IT140_TRANSACTION_ROOT/records.tsv"
    IT140_TRANSACTION_ACTIVE=0
    rm -rf "$IT140_TRANSACTION_ROOT"
    IT140_TRANSACTION_ROOT=""
}
...
if configuration_complete; then
    ...
    workflow="First use or reset environment"
fi
it140_info "Workflow         : $workflow"
it140_check_free_space
it140_acquire_lock

it140_header "Step 1: Retrieve and Validate Candidate Course Assets"
IT140_TEMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/it140-update.XXXXXX")"
candidate_repo="$IT140_TEMP_ROOT/it140"
git clone --depth 1 --quiet "$IT140_REPOSITORY_URL" "$candidate_repo"
candidate_manifest="$candidate_repo/scripts/.manifest/it140_manifest.json"
candidate_schema="$candidate_repo/scripts/.manifest/it140_manifest.schema.json"
...
for script_name in config_mac.sh update_mac.sh verify_mac.sh; do
    ...
    candidate_version="$(extract_script_version "$candidate_script")"
    printf '%s\n' "$candidate_version" | grep -Eq '^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$' || it140_fail 5 "$script_name does not declare three-part SemVer."
    installed_script="$IT140_PLATFORM_SCRIPT_DIR/$script_name"
    if [ -r "$installed_script" ]; then
        installed_version="$(extract_script_version "$installed_script")"
        if printf '%s\n' "$installed_version" | grep -Eq '^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$' && [ "$(semver_compare "$candidate_version" "$installed_version")" -lt 0 ]; then
            it140_fail 5 "$script_name $candidate_version would downgrade installed version $installed_version."
        fi
    fi
done
[ ! -e "$candidate_repo/scripts/mac/_mac_common.sh" ] || it140_fail 5 "Candidate release unexpectedly contains the retired _mac_common.sh runtime dependency."
it140_success "Candidate release $candidate_release passed controlled-file and script validation."

it140_header "Step 2: Activate Candidate Student Assets Atomically"
begin_transaction
...
active_summary="$(it140_json_validate)"
it140_success "Course-managed student assets are synchronized at manifest release ${active_summary%%$'\t'*}."
IT140_CHANGED=1

it140_header "Step 3: Update Course IDE Software"
it140_initialize_homebrew
...
while IFS= read -r formula; do
    ...
    if brew list --formula "$formula" >/dev/null 2>&1; then
        if brew outdated --formula "$formula" | grep -Fxq "$formula"; then brew upgrade "$formula"; IT140_CHANGED=1; else it140_success "Homebrew formula is current: $formula"; fi
    else
...
while IFS= read -r cask; do
    ...
    if brew list --cask "$cask" >/dev/null 2>&1; then
        if brew outdated --cask "$cask" | grep -Fxq "$cask"; then brew upgrade --cask "$cask"; IT140_CHANGED=1; else it140_success "Homebrew cask is current: $cask"; fi
    else
...
it140_header "Step 4: Update User-Level Course Components"
if [ ! -x "$IT140_VENV_PYTHON" ]; then
    it140_info "Creating the missing course Python 3.12 virtual environment."
...
while IFS= read -r package; do
    [ -n "$package" ] || continue
    "$IT140_VENV_PYTHON" -m pip install --upgrade "$package"
done < <(it140_manifest_query venv_packages)
IT140_CHANGED=1
if [ -x "$IT140_CODE_BIN" ]; then
    "$IT140_CODE_BIN" --update-extensions || it140_warning "One or more optional VS Code extension updates reported a warning."
    while IFS= read -r extension; do
        [ -n "$extension" ] || continue
        "$IT140_CODE_BIN" --install-extension "$extension" --force
    done < <(it140_manifest_query extensions)
    IT140_CHANGED=1
else
    it140_warning "Visual Studio Code is unavailable; extensions were not updated."
    IT140_PARTIAL=1
fi

it140_header "Step 5: Verify the Maintained Environment"
it140_system_layer_available || it140_fail 7 "A required system component is unavailable after update."
while IFS= read -r package; do
    [ -n "$package" ] || continue
...
if [ "$IT140_PARTIAL" -eq 1 ]; then result="PARTIAL"; exit_code=7; else result="PASS"; exit_code=0; fi
it140_info "Result           : $result"
...
it140_notice "Next step: $next_step"
it140_closing_notice
exit "$exit_code"
