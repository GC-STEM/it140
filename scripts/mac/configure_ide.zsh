...
# ==============================================================================
# Repository path: scripts/mac/configure_ide.zsh
# Purpose: Configure or repair the current macOS user layer for the IT 140 course IDE.
...
# Traceability: CFG-FR-001 through CFG-FR-016; PKG-FR-003 through PKG-FR-010; PKG-FR-021; PKG-QOS-011 through PKG-QOS-015.
# ==============================================================================

set -euo pipefail
umask 077

readonly IT140_ACTION='configure'
readonly IT140_ACTION_DISPLAY='Configure'
readonly IT140_ARTIFACT_ID='IT140-MAC-CONFIGURE'
...
Configures the current user’s course folders, PATH, Python environment, GitHub authentication, privacy-preserving Git identity, VS Code extensions and settings, and desktop integration.
...
it140_initialize_log() {
    ...
    printf '[INFO] Purpose                 : %s\n' 'Configure or repair the current macOS user layer for the IT 140 course IDE.'
...
it140_header 'Stage 1: System-Layer Prerequisites'
IT140_CURRENT_STAGE='Verify the installed system layer'
BREW_PATH=''
if ! BREW_PATH="$(it140_find_brew)"; then
    it140_abort 1 'Homebrew is unavailable. Run install_ide.zsh first.'
fi
...
while IFS= read -r command_name; do
    ...
    command -v "$command_name" >/dev/null 2>&1 || \
        it140_abort 1 "Required system command is missing: ${command_name}. Run install_ide.zsh."
done < "$COMMANDS_FILE"

PYTHON_COMMAND="$(it140_manifest_tool python_executable)"
PYTHON_PATH="$(command -v "$PYTHON_COMMAND" 2>/dev/null || true)"
[[ -n "$PYTHON_PATH" ]] || it140_abort 1 "Required Python command is missing: ${PYTHON_COMMAND}. Run install_ide.zsh."

it140_header 'Stage 2: Course Folders, PATH, and Python Environment'
IT140_CURRENT_STAGE='Create required user folders'
/bin/mkdir -p -- "$IT140_COURSE_ROOT" "$IT140_LOG_DIR"
/bin/chmod -- 0700 "$IT140_LOG_DIR"
it140_replace_managed_environment_block

IT140_CURRENT_STAGE='Create or repair the course Python virtual environment'
if [[ ! -x "$IT140_VENV_DIR/bin/python" ]]; then
    if ! "$PYTHON_PATH" -m venv "$IT140_VENV_DIR"; then
        it140_abort 7 'The course Python virtual environment could not be created.'
    fi
    IT140_CHANGED=true
fi
VENV_PYTHON="$IT140_VENV_DIR/bin/python"
if ! "$VENV_PYTHON" -m pip install --upgrade pip; then
    it140_abort 4 'pip could not be updated from the approved Python package source.'
fi
VENV_PACKAGES_FILE="$(it140_make_list_file venv_packages)"
while IFS= read -r package_id; do
    [[ -n "$package_id" ]] || continue
    IT140_CURRENT_STAGE="Install course Python package ${package_id}"
    if ! "$VENV_PYTHON" -m pip install --upgrade "$package_id"; then
        it140_abort 4 "Course Python package could not be installed: ${package_id}."
    fi
    IT140_CHANGED=true
done < "$VENV_PACKAGES_FILE"

it140_header 'Stage 3: GitHub Authentication and Git Identity'
IT140_CURRENT_STAGE='Verify GitHub authentication'
if ! gh auth status --hostname github.com >/dev/null 2>&1; then
    if [[ "$IT140_NONINTERACTIVE" == true ]]; then
        it140_abort 6 'GitHub authentication requires an interactive Configure run.'
    fi
    it140_notice 'A browser-based GitHub sign-in is required. Complete the sign-in, then return to Terminal.'
    if ! gh auth login --hostname github.com --git-protocol https --web --clipboard \
            </dev/tty >/dev/tty 2>/dev/tty; then
        it140_abort 6 'GitHub authentication was canceled or did not complete.'
    fi
fi
if ! gh auth status --hostname github.com >/dev/null 2>&1; then
    it140_abort 1 'GitHub authentication could not be verified.'
fi
GITHUB_USERNAME="$(gh api user --jq '.login' 2>/dev/null || true)"
GITHUB_ACCOUNT_ID="$(gh api user --jq '.id' 2>/dev/null || true)"
GITHUB_DISPLAY_NAME="$(gh api user --jq '.name // empty' 2>/dev/null || true)"
[[ -n "$GITHUB_USERNAME" && "$GITHUB_ACCOUNT_ID" =~ ^[0-9]+$ ]] || \
    it140_abort 1 'The required GitHub account fields could not be retrieved.'

GIT_DISPLAY_NAME="$GITHUB_DISPLAY_NAME"
[[ -n "$GIT_DISPLAY_NAME" ]] || GIT_DISPLAY_NAME="$GITHUB_USERNAME"
if [[ "$IT140_NONINTERACTIVE" != true ]]; then
    it140_notice 'The Git display name will appear in version-control history. Your email address will use GitHub’s privacy-preserving noreply format.'
    printf 'Press Return to use "%s", or type another public display name: ' "$GIT_DISPLAY_NAME" >/dev/tty
    IFS= read -r RESPONSE </dev/tty || it140_abort 6 'Git display-name entry was canceled.'
    [[ -z "$RESPONSE" ]] || GIT_DISPLAY_NAME="$RESPONSE"
fi
if [[ -z "$GIT_DISPLAY_NAME" || "$GIT_DISPLAY_NAME" == *$'\n'* || ${#GIT_DISPLAY_NAME} -gt 100 ]]; then
    it140_abort 2 'The Git display name is invalid.'
fi
PRIVATE_EMAIL="${GITHUB_ACCOUNT_ID}+${GITHUB_USERNAME}@users.noreply.github.com"
...
it140_success "GitHub authentication and privacy-preserving Git identity are configured for ${GITHUB_USERNAME}."
unset PRIVATE_EMAIL

it140_header 'Stage 4: Visual Studio Code Extensions and Settings'
IT140_CURRENT_STAGE='Install required Visual Studio Code extensions'
...
while IFS= read -r extension_id; do
    ...
    if ! code --install-extension "$extension_id" --force; then
        it140_abort 4 "Required Visual Studio Code extension could not be installed: ${extension_id}."
    fi
    IT140_CHANGED=true
done < "$EXTENSIONS_FILE"

IT140_CURRENT_STAGE='Merge course-managed Visual Studio Code settings'
it140_merge_vscode_settings "$VENV_PYTHON"
it140_configure_workspace_settings "$VENV_PYTHON"

it140_header 'Stage 5: Desktop Integration'
IT140_CURRENT_STAGE='Create the course-folder shortcut'
DESKTOP_DIR="$HOME/Desktop"
COURSE_LINK="$DESKTOP_DIR/IT 140"
IDE_APP="$DESKTOP_DIR/IT 140 Course IDE.app"
if [[ -d "$DESKTOP_DIR" ]]; then
    if [[ -L "$COURSE_LINK" ]]; then
        /bin/ln -sfn "$IT140_COURSE_ROOT" "$COURSE_LINK"
        IT140_CHANGED=true
    elif [[ ! -e "$COURSE_LINK" ]]; then
        /bin/ln -s "$IT140_COURSE_ROOT" "$COURSE_LINK"
        IT140_CHANGED=true
    else
        it140_warning 'Desktop item “IT 140” already exists and is not a course-managed symbolic link; it was preserved.'
    fi

    if [[ -e "$IDE_APP" && ! -f "$IDE_APP/Contents/Resources/it140-managed" ]]; then
        it140_warning 'Desktop item “IT 140 Course IDE.app” is not recognized as course-managed; it was preserved.'
    else
        /bin/rm -rf -- "$IDE_APP"
        /bin/mkdir -p -- "$IDE_APP/Contents/MacOS" "$IDE_APP/Contents/Resources"
        cat > "$IDE_APP/Contents/Info.plist" <<'PLIST'
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
<key>CFBundleDisplayName</key><string>IT 140 Course IDE</string>
<key>CFBundleExecutable</key><string>open-it140</string>
<key>CFBundleIdentifier</key><string>edu.snhu.it140.course-ide</string>
<key>CFBundlePackageType</key><string>APPL</string>
<key>CFBundleVersion</key><string>1</string>
</dict></plist>
PLIST
        cat > "$IDE_APP/Contents/MacOS/open-it140" <<'LAUNCH'
#!/bin/zsh
set -eu
course_root="$HOME/it140"
if command -v code >/dev/null 2>&1; then
    exec "$(command -v code)" --reuse-window "$course_root"
fi
code_cli='/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code'
if [[ -x "$code_cli" ]]; then
    exec "$code_cli" --reuse-window "$course_root"
fi
/usr/bin/open -a 'Visual Studio Code' "$course_root"
LAUNCH
        : > "$IDE_APP/Contents/Resources/it140-managed"
        /bin/chmod -- 0755 "$IDE_APP/Contents/MacOS/open-it140"
        /usr/bin/plutil -lint "$IDE_APP/Contents/Info.plist" >/dev/null
        IT140_CHANGED=true
    fi
else
    it140_warning 'The Desktop folder is unavailable; desktop integration was skipped.'
fi

it140_header 'Stage 6: Post-Configuration Validation'
IT140_CURRENT_STAGE='Validate the configured user environment'
"$VENV_PYTHON" -c 'import pytest, pytest_cov' >/dev/null
for required_command in git gh code; do
    command -v "$required_command" >/dev/null 2>&1 || \
        it140_abort 1 "Required command is unavailable after configuration: ${required_command}."
done

git config --global user.name >/dev/null
git config --global user.email | /usr/bin/grep -Eq '^[0-9]+\+[^@[:space:]]+@users\.noreply\.github\.com$' || \
    it140_abort 1 'The privacy-preserving GitHub noreply identity did not pass validation.'

it140_finish 0 'PASS' \
    'The current user environment, GitHub integration, course tools, IDE settings, extensions, and desktop integration are configured.' \
