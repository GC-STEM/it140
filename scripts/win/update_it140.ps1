...
$ScriptVersion = "1.0.4"
$VersionDateTimeGroup = "2026-09-07-14-30"
...
Usage:
  powershell.exe -ExecutionPolicy Bypass -File .\update_it140.ps1
  powershell.exe -ExecutionPolicy Bypass -File .\update_it140.ps1 -NonInteractive
  powershell.exe -ExecutionPolicy Bypass -File .\update_it140.ps1 -Help
  powershell.exe -ExecutionPolicy Bypass -File .\update_it140.ps1 -Version
...
function Update-ManagedVsCodeSetting {
    ...
    if (-not (Test-Path -LiteralPath $VsCodeSettings -PathType Leaf)) {
        Write-Notice "VS Code settings are not configured; configure_it140.ps1 will create them."
        return
    }
...
function Update-ManagedIntegration {
    ...
    if (
        ...
        Write-Notice "Desktop shortcuts are not configured; configure_it140.ps1 will create them."
    }
}
...
try {
    ...
    if (Test-IsAdministrator) {
        $FailureExitCode = 3
        throw (
            "Run update_it140.ps1 from a normal, non-elevated PowerShell window. " +
