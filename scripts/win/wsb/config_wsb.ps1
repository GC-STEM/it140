...
<#
.SYNOPSIS
Configures or repairs the Windows Sandbox user for the IT 140 Course IDE.
...
Run this script only inside Windows Sandbox. Windows Sandbox intentionally
uses its administrative WDAGUtilityAccount container context.
...
$LogPath = Join-Path $LogDirectory (
    "config_{0}_{1}.log" -f $LogAbbreviation, (Get-Date -Format "yyyyMMdd_HHmmss")
)
...
function Show-Usage {
    @"
IT 140 Windows Sandbox configure script

Usage:
  powershell.exe -ExecutionPolicy Bypass -File .\config_wsb.ps1
  powershell.exe -ExecutionPolicy Bypass -File .\config_wsb.ps1 -NonInteractive
  powershell.exe -ExecutionPolicy Bypass -File .\config_wsb.ps1 -Help
  powershell.exe -ExecutionPolicy Bypass -File .\config_wsb.ps1 -Version

Run this script only in a Windows Sandbox PowerShell window. The
administrative WDAGUtilityAccount container context is required and expected.
...
function Set-ManagedUserPath {
    $ManagedEntries = @($VenvScriptsDirectory, $WindowsSandboxScriptDirectory)
    $RetiredEntries = @($WindowsScriptDirectory)
    ...
    foreach ($ExistingEntry in @($ExistingUserPath -split ";")) {
        ...
        foreach ($ManagedEntry in @($ManagedEntries + $RetiredEntries)) {
            if (
    ...
    Write-Success "The course Python and Windows Sandbox script directories are first in the user PATH."
}
...
function Test-SystemLayer {
    ...
    if ($MissingCommands.Count -gt 0) {
        throw (
            "Required system commands are missing: {0}. Run setup_wsb.ps1." -f
    ...
    if ($LASTEXITCODE -ne 0 -or [string]$PythonVersion -ne "3.12") {
        throw "Python 3.12 is not the active Windows system runtime. Run setup_wsb.ps1."
    }
...
function Test-ConfiguredUserLayer {
    ...
    $VerifyScript = Join-Path $WindowsSandboxScriptDirectory "verify_wsb.ps1"
...
try {
    ...
    Write-Header "IT 140 WINDOWS SANDBOX CONFIGURATION"
    ...
    Write-Notice "This script changes only the Windows Sandbox user's IT 140 environment."
    ...
    if (-not $IsWindowsSandbox) {
        $FailureExitCode = 2
        throw "config_wsb.ps1 can be used only inside Windows Sandbox."
    }
    if (-not $IsAdministrator) {
        $FailureExitCode = 3
        throw (
            "Windows Sandbox requires its administrative WDAGUtilityAccount " +
            "container context."
        )
    }

    Write-Notice (
        "Windows Sandbox uses its administrative WDAGUtilityAccount container " +
        "context. This context is expected."
    )
    ...
    Write-Success "The Windows Sandbox user is configured for IT 140."
    ...
    if (
        ...
        $null -ne $_.InvocationInfo.PSObject.Properties["ScriptLineNumber"]
    ) {
    ...
    if ($Changed) {
        Write-Notice (
            ...
            "Rerun config_wsb.ps1 to repair it."
        )
        $ExitCode = 7
    }
    else {
