...
<#
.SYNOPSIS
Verifies the Windows Sandbox IT 140 Course IDE without repairing it.
...
The script never elevates privilege or repairs failed checks. It runs only
inside the administrative Windows Sandbox WDAGUtilityAccount container context.
...
$ScriptVersion = "2026.07.28.2"
$PlatformId = "windows"
$PlatformAbbreviation = "win"
$LogAbbreviation = "wsb"
$WindowsSandboxScriptDirectory = $PSScriptRoot
$WindowsScriptDirectory = Split-Path -Parent $WindowsSandboxScriptDirectory
$ScriptRoot = Split-Path -Parent $WindowsScriptDirectory
$CourseRoot = Split-Path -Parent $ScriptRoot
$ScriptDirectory = $WindowsSandboxScriptDirectory
...
$LogPath = Join-Path $LogDirectory (
    "verify_{0}_{1}.log" -f $LogAbbreviation, (Get-Date -Format "yyyyMMdd_HHmmss")
)
...
$DeploymentProfile = "windows_sandbox"
...
function Show-Usage {
    @"
IT 140 Windows Sandbox verification script

Usage:
  powershell.exe -ExecutionPolicy Bypass -File .\verify_wsb.ps1
  powershell.exe -ExecutionPolicy Bypass -File .\verify_wsb.ps1 -SkipNetwork
  powershell.exe -ExecutionPolicy Bypass -File .\verify_wsb.ps1 -SupportBundle
  powershell.exe -ExecutionPolicy Bypass -File .\verify_wsb.ps1 -SupportBundle -Yes
  powershell.exe -ExecutionPolicy Bypass -File .\verify_wsb.ps1 -Help
  powershell.exe -ExecutionPolicy Bypass -File .\verify_wsb.ps1 -Version

This script does not elevate privilege, install, repair, update, remove, or
rewrite managed course state. Run this script only in the expected administrative Windows Sandbox
WDAGUtilityAccount container context.
Support-bundle creation requires explicit confirmation unless -Yes is supplied.

Deployment profile: windows_sandbox
...
function Test-PlatformContext {
    ...
    if (-not $IsWindowsSandbox) {
        $script:UnsupportedFailure = $true
        Add-CheckResult `
            -CheckId "verify.deployment_context" `
            -Status "FAIL" `
            -Detail "verify_wsb.ps1 was run outside Windows Sandbox" `
            -Remediation "Start Windows Sandbox with it140_wsb.wsb."
    }
    else {
        Add-CheckResult `
            -CheckId "verify.deployment_context" `
            -Status "PASS" `
            -Detail "Windows Sandbox WDAGUtilityAccount environment detected"
    }
    ...
    if ($FreeBytes -ge $MinimumBytes) {
        ...
        Add-CheckResult `
            ...
            -Remediation (
                ...
                "free, then rerun verify_wsb.ps1."
            )
    }
...
function Test-UserLayer {
    ...
    $FoldersPresent = $true
    foreach ($RequiredDirectory in @($CourseRoot, $LogDirectory, $WindowsSandboxScriptDirectory)) {
    ...
    if ($FoldersPresent) {
        Add-CheckResult `
            ...
            -Status "PASS" `
            -Detail "Course root, log, and Windows Sandbox script directories are present"
    }
    else {
        Add-CheckResult `
            -CheckId "verify.course_folders" `
    ...
    $UserPathReady = $true
    foreach ($ManagedPath in @($VenvScriptsDirectory, $WindowsSandboxScriptDirectory)) {
    ...
    if ($UserPathReady) {
        Add-CheckResult `
            ...
            -Status "PASS" `
            -Detail "Course Python and Windows Sandbox script directories are in the user PATH"
    }
    else {
        Add-CheckResult `
            -CheckId "verify.user_path" `
    ...
    if ($CurrentPathReady) {
        ...
        Add-CheckResult `
            ...
            -Remediation "Close this PowerShell window, open a new one, and rerun verify_wsb.ps1."
    }
        ...
        try {
            ...
            Add-CheckResult `
                ...
                -Remediation (
                    ...
                    "support before rerunning config_wsb.ps1."
                )
        }
    }
    else {
    ...
    $LifecycleScripts = @(
        [pscustomobject]@{
            ...
            Path = Join-Path $WindowsSandboxScriptDirectory "bootstrap_wsb.ps1"
        },
        [pscustomobject]@{
            Name = "setup_wsb.ps1"
            Path = Join-Path $WindowsSandboxScriptDirectory "setup_wsb.ps1"
        },
        [pscustomobject]@{
            Name = "config_wsb.ps1"
            Path = Join-Path $WindowsSandboxScriptDirectory "config_wsb.ps1"
        },
        [pscustomobject]@{
            Name = "verify_wsb.ps1"
            Path = Join-Path $WindowsSandboxScriptDirectory "verify_wsb.ps1"
        }
    )
...
function Write-VerificationSummary {
    ...
    if ($ResolvedExitCode -eq 0) {
        ...
        Write-Notice "Complete the listed remediation, then rerun verify_wsb.ps1."
    }
...
function New-SupportBundle {
    ...
    try {
        if (Test-Path -LiteralPath $LogPath -PathType Leaf) {
            ...
            Write-Utf8LfFile `
                -Path (Join-Path $StagingDirectory "verify_wsb_sanitized.log") `
        ...
        $ReleaseSummary = [pscustomobject]@{
            ...
            ConfigScriptVersion = $LifecycleVersionSummary["config_wsb.ps1"]
            UpdateScriptVersion = "not applicable"
            VerifyScriptVersion = $LifecycleVersionSummary["verify_wsb.ps1"]
...
try {
    ...
    Write-Header "IT 140 WINDOWS SANDBOX VERIFICATION"
    Write-Info "Script version   : $ScriptVersion"
    Write-Info "Deployment       : $DeploymentProfile"
    Write-Info "Sandbox user     : $([Environment]::UserName)"
