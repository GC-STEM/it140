...
4. When setup succeeds, a normal Windows PowerShell continuation window opens and displays:
    ...
    After configuration, close this window, open the desktop shortcut again, and run verify_win.ps1.
    ```
...
Do not run `update_win.ps1` in Windows Sandbox. Every sandbox launch starts from a fresh Windows image, so periodic maintenance does not apply.
